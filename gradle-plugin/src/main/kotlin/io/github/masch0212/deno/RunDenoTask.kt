...
open class RunDenoTask @Inject constructor(private val execOperations: ExecOperations) :
    DefaultTask() {
  ...
  @Input
  @Optional
  var version: String = project.extensions.getByType(DenoExtension::class.java).version.get()

  @Input var command: String = "--version"
  ...
  fun runCommand() {
    ...
    if (!denoExecutable.exists()) {
      throw IllegalStateException(
          "❌ Deno $denoVersion is not installed. Run `./gradlew installDeno` first.")
    }
    ...
    execOperations.exec { commandLine(denoExecutable.absolutePath, *commandParts.toTypedArray()) }
  }
